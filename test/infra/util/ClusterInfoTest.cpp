...
#include "../../../src/infra/util/Cluster.h"
#include "../../../src/app_util/AppInfo.h"
#include "../../../src/infra/util/Util.h"

using gringofts::Cluster;
using gringofts::app::AppClusterParser;

TEST(ClusterInfoTest, routeInfoTest) {
  EXPECT_FALSE(AppClusterParser::checkHasRoute("aa", 0, 0));
  EXPECT_FALSE(AppClusterParser::checkHasRoute("{\"epoch\":\"a1\"}", 0, 0));
  EXPECT_TRUE(AppClusterParser::checkHasRoute(
  ...
  EXPECT_FALSE(AppClusterParser::checkHasRoute(
      "{\"epoch\":1,\"routeInfos\":[{\"clusterId\":0,\"routeEntries\":[{\"groups\":[0,1,2,3,4,5,6,7],\"groupTotal\":8}]},{\"clusterId\":1,\"routeEntries\":[{\"groups\":[0,1,2,3,4,5,6,7],\"groupTotal\":8}]}]}",  // NOLINT
      2,
      0));
}
...
TEST(ClusterInfoTest, resolveAllClustersUsingExternal) {
  ...
  using gringofts::app::RouteSignal;
  {
    ExternalClientMock::mKv["cluster.conf"] =
        absl::StrFormat("1#1@node11.ebay.com,2@%s,3@node13.ebay.com", gringofts::Util::getHostname());
    auto parser = std::make_unique<AppClusterParser>(std::make_unique<ClientFactory_t<ExternalClientMock>>());
    auto[clusterId, nodeId, allClusterInfo]
         = parser->parse(INIReader("../test/infra/util/config/cluster_external.ini"));
    EXPECT_EQ(clusterId, 1);
    EXPECT_EQ(nodeId, 2);
    auto routeSignal = std::make_shared<gringofts::app::RouteSignal>(1, 1);
  ...
  }
  {
    ...
    ExternalClientMock::mKv["cluster.route"] =
        "{\"epoch\":1,\"routeInfos\":[{\"clusterId\":0,\"routeEntries\":[{\"groups\":[0,1,2,3,4,5,6,7],\"groupTotal\":8}]},{\"clusterId\":1,\"routeEntries\":[{\"groups\":[0,1,2,3,4,5,6,7],\"groupTotal\":8}]}]}";  // NOLINT
    auto parser
        = std::make_unique<AppClusterParser>(std::make_unique<ClientFactory_t<ExternalClientMock>>());
    auto[clusterId, nodeId, allClusterInfo] =
        parser->parse(INIReader("../test/infra/util/config/cluster_external.ini"));
    EXPECT_EQ(clusterId, 1);
    EXPECT_EQ(nodeId, 2);
