...
NewDeleteMemoryPool::NewDeleteMemoryPool(const std::string &name):
  PMRMemoryPool(name) {
  mInternalMemoryResource = std::unique_ptr<std::pmr::memory_resource>(std::pmr::new_delete_resource());
  mMemoryResource = std::make_unique<TrackingMemoryResource>(name, mInternalMemoryResource.get());
}
