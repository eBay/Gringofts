...
class Log {
 public:
  ...
  /// append
  virtual bool appendEntry(const gringofts::raft::LogEntry &entry) = 0;
  virtual bool appendEntries(const std::vector<gringofts::raft::LogEntry> &entries) = 0;

  /// kinds of get
  virtual bool getEntry(uint64_t index, gringofts::raft::LogEntry *entry) const = 0;
  ...
  virtual bool getEntries(uint64_t index, uint64_t size,
                          gringofts::raft::LogEntry *entries) const = 0;
  ...
  virtual uint64_t getEntries(const uint64_t startIndex,
                              ...
                              std::vector<gringofts::raft::LogEntry> *entries) const = 0;
 ...
 public:
   ...
   */
  void append(const gringofts::raft::LogEntry &entry) {
    assert(appendEntry(entry));
  }
  void append(const std::vector<gringofts::raft::LogEntry> &entries) {
