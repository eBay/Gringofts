...
class Segment final {
 public:
  ...
  bool shouldRoll(const std::vector<gringofts::raft::LogEntry> &entries) const;

  /// require: call shouldRoll() ahead to make sure that
  ///          current Segment has enough space to hold entries.
  void appendEntries(const std::vector<gringofts::raft::LogEntry> &entries);

  /// kinds of get method
  bool getEntry(uint64_t index, gringofts::raft::LogEntry *entry) const;
  bool getTerm(uint64_t index, uint64_t *term) const;

  bool getEntries(uint64_t index, uint64_t size, gringofts::raft::LogEntry *entries) const;
